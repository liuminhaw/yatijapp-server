...
func ValidateSession(v *validator.Validator, session *Session) {
	v.Check(session.ActivityUUID != uuid.Nil, "activity_uuid", "must be provided")
...
func (m SessionModel) Insert(session *Session, fts FTS, userUUID uuid.UUID) error {
	...
	new_session AS (
		INSERT INTO sessions (activity_uuid)
		SELECT a.uuid
		FROM activities a 
	...
	args := []any{session.ActivityUUID, userUUID, fts.NotesToken.Chinese, fts.NotesToken.English}
...
func (m SessionModel) Get(uuid, userUUID uuid.UUID, minRole string) (*Session, error) {
	query := `
		SELECT 
			...
			t.title
		FROM sessions s
		JOIN activities a ON s.activity_uuid = a.uuid
	...
	query := `
		...
		update_session AS (
			...
			WHERE s.uuid = $5 AND s.version = $6 AND (
				(
					$4 IS NOT DISTINCT FROM s.activity_uuid AND (
						EXISTS (
							SELECT 1
							FROM acls ac
							JOIN roles r ON ac.role_code = r.code
						...
						EXISTS (
							...
							AND ac.resource_uuid = s.activity_uuid
							AND ac.user_uuid = $7
						)
						OR
						EXISTS (
				...
				OR
				(
					$4 IS DISTINCT FROM s.activity_uuid AND EXISTS (
						SELECT 1
						FROM acls ac
						JOIN roles r ON ac.role_code = r.code
...
func (m SessionModel) GetAll(
	...
	// 		ELSE 0 END) AS rank
	// 	FROM sessions s
	// 	JOIN sessions_ftsfts ON s.uuid = fts.session_uuid
	// 	JOIN activities a ON s.activity_uuid = a.uuid
	// 	JOIN targets t ON a.target_uuid = t.uuid
	...
	// 		AND ($3::uuid IS NULL OR s.activity_uuid = $3)
	// 		AND EXISTS (
	// 			SELECT 1
	// 			FROM acls ac
	// 			JOIN roles r ON ac.role_code = r.code
	...
	query := fmt.Sprintf(`
		WITH filtered AS MATERIALIZED (
			SELECT s.uuid, s.activity_uuid, a.target_uuid
			FROM sessions s
		...
		JOIN LATERAL (
			...
			WHERE ac.user_uuid = $4
				AND (
					...
					(ac.resource_type = 'activity' AND ac.resource_uuid = p.activity_uuid) OR
